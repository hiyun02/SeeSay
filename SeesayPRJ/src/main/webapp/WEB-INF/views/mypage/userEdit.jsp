		...
		<div class="d-flex flex-column align-items-center justify-content-center" style="padding-top: 5%">
			<div class="card card-bg p-4" style="box-shadow: 1px 1px 3px 3px">
				<div class="login-title">
					<h2 class="text-center text-primary" style="padding-bottom: 10%">사용자 정보 수정</h2>
				</div>
				<form action="/mypage/userEditProc" method="post">

					<div class="input-group custom">
						<label for="inputName" class="form-label">아이디</label>
						<input class="form-control rounded-pill border-white input-box" name="userId" id="inputID" type="text" value="<%=userDTO.getUserId()%>" READONLY />
					</div>

					<div class="input-group custom">
						<label for="inputName" class="form-label">이름</label>
						<input class="form-control rounded-pill border-white input-box" id="inputName" name="userName" type="text" placeholder="이름을 입력해주세요" value="<%=userDTO.getUserName()%>" />
						<div class="invalid-feedback">이름을 입력해주세요</div>
					</div>
					<div class="input-group custom">
						<label for="inputEmail" class="form-label">이메일</label>
						<input class="form-control rounded-pill border-white input-box" id="inputEmail" name="email" type="email" placeholder="이메일을 입력해주세요" value="<%=userDTO.getEmail()%>"/>
						<div class="invalid-feedback">이메일을 입력해주세요</div>
					</div>
					<div class="input-group custom">
						<label for="inputEmail" class="form-label">보호자 전화번호</label>
						<input class="form-control rounded-pill border-white" id="inputnumber" type="text" name="phoneNumber" placeholder="보호자의 전화번호를 입력해주세요" value="<%=userDTO.getPhoneNumber()%>"/>

						<div class="invalid-feedback">보호자의 전화번호를 입력해주세요</div>
